class build-couchdb::build {
   ...
   exec { "install packages git make gcc zlib1g-dev libssl-dev rake":
       command => "sudo apt-get install git make gcc zlib1g-dev libssl-dev rake -y --force-yes",
   ...
   exec { "build couchdb":
       ...
       refreshonly => true,
       subscribe => Exec["install packages git make gcc zlib1g-dev libssl-dev rake"],
